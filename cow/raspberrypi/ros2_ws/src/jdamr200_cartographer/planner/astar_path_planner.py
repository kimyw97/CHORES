...
from scipy.interpolate import splprep, splev  # 경로 smoothing
...
class AStarPlanner(Node):
    def __init__(self):
        ...
        self.start = (int(w / 2), int(h / 2) + 20)  # 맵 중심에서 살짝 위쪽 (빈 공간일 확률 높음)
        self.goal = None
    ...
    def load_map(self):
        ...
        # === 장애물 영역 확장 ===
        kernel = np.ones((2, 2), np.uint8)  # 3x3 커널 (크면 더 많이 확장됨)
        dilated = cv2.dilate(255 - image, kernel, iterations=1)  # 반전 → 확장 → 다시 반전
        image = 255 - dilated
    ...
    def smooth_path(self, path, smoothing_factor=3.0, num_points=100):
        if len(path) < 4:
            return path  # 점이 너무 적으면 smoothing 안 함
